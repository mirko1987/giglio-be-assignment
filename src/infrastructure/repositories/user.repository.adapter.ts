...
export class UserRepositoryAdapter implements UserRepositoryPort {
  ...
  findById(id: string): Observable<User | null> {
    return from(this.userRepository.findOne({ where: { id } })).pipe(
      ...
      catchError(error => throwError(() => new Error(`Failed to find user by ID: ${error.message}`)))
    );
  }
  ...
  findByEmail(email: string): Observable<User | null> {
    return from(this.userRepository.findOne({ where: { email } })).pipe(
      map(userOrm => userOrm ? UserMapper.toDomainEntity(userOrm) : null),
      catchError(error => throwError(() => new Error(`Failed to find user by email: ${error.message}`)))
    );
  }
  ...
  findAll(): Observable<User[]> {
    return from(this.userRepository.find({ order: { createdAt: 'DESC' } })).pipe(
      ...
      catchError(error => throwError(() => new Error(`Failed to find all users: ${error.message}`)))
    );
  }
  ...
  exists(id: string): Observable<boolean> {
    return from(this.userRepository.exists({ where: { id } })).pipe(
      catchError(error => throwError(() => new Error(`Failed to check user existence: ${error.message}`)))
    );
  }
}
