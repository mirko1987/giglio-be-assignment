...
export class OrderRepositoryAdapter implements OrderRepositoryPort {
  ...
  save(order: Order): Observable<Order> {
    ...
    return from(this.orderRepository.save(orderOrm)).pipe(
      map(savedOrderOrm => OrderMapper.toDomainEntity(savedOrderOrm)),
  ...
  findById(id: string): Observable<Order | null> {
    return from(this.orderRepository.findOne({
      ...
      catchError(error => throwError(() => new Error(`Failed to find order by ID: ${error.message}`)))
    );
  }
  ...
  findByUserId(userId: string): Observable<Order[]> {
    return from(this.orderRepository.find({
      ...
      catchError(error => throwError(() => new Error(`Failed to find orders by customer ID: ${error.message}`)))
    );
  }
  ...
  findByStatus(status: string): Observable<Order[]> {
    return from(this.orderRepository.find({
      ...
      map(ordersOrm => ordersOrm.map(orderOrm => OrderMapper.toDomainEntity(orderOrm))),
      catchError(error => throwError(() => new Error(`Failed to find orders by status: ${error.message}`)))
    );
  }
  ...
  findAll(): Observable<Order[]> {
    return from(this.orderRepository.find({
      ...
      map(ordersOrm => ordersOrm.map(orderOrm => OrderMapper.toDomainEntity(orderOrm))),
      catchError(error => throwError(() => new Error(`Failed to find all orders: ${error.message}`)))
    );
  }
  ...
  delete(id: string): Observable<void> {
    return from(this.orderRepository.delete(id)).pipe(
      ...
      catchError(error => throwError(() => new Error(`Failed to delete order: ${error.message}`)))
    );
  }

  exists(id: string): Observable<boolean> {
    return from(this.orderRepository.exists({ where: { id } })).pipe(
      catchError(error => throwError(() => new Error(`Failed to check order existence: ${error.message}`)))
    );
  }
}
