...
export class OrderStatusChangedHandler {
  ...
  private handleSpecificStatusChanges(event: OrderStatusChangedEvent): void {
    switch (event.newStatus) {
      case 'SHIPPED':
        this.logger.log(
          `Order ${event.orderId} has been shipped - triggering shipping notifications`,
        );
        break;
      case 'DELIVERED':
        this.logger.log(
          `Order ${event.orderId} has been delivered - triggering delivery confirmations`,
        );
        break;
      case 'CANCELLED':
      ...
      case 'DELIVERED':
        this.logger.log(
          `Order ${event.orderId} has been completed - triggering completion workflows`,
        );
        break;
      default:
