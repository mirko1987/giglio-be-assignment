...
export class UpdateOrderStatusUseCase {
    ...
    return this.findOrder(request.orderId).pipe(
      ...
      switchMap((order) =>
        this.updateOrderStatus(
          order,
          typeof request.newStatus === 'string'
            ? OrderStatus[request.newStatus as keyof typeof OrderStatus]
            : request.newStatus,
        ),
      ),
  ...
  private sendNotification(order: Order): Observable<Order> {
    return this.notificationService
      .sendOrderStatusUpdate(order.user.email.value, order.id, order.status.toString())
      .pipe(switchMap(() => of(order)));
  }

  private mapToResponse(order: Order, _newStatus: OrderStatus | string): UpdateOrderStatusResponse {
    return {
      orderId: order.id,
